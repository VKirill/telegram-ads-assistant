import {destinationKind} from './destinations.js';
...
export function validateDetails(p,constraints=defaultConstraints){
 ...
 for(const key of Object.keys(a))if(!['id','title','text','url','budget','cpm','status','target','channels','externalId','media','dailyBudget','viewsPerUser','picture','websiteName'].includes(key))add(base+'.'+key,'unknown_field','Неизвестное поле: '+key);
 ...
 for(const link of links){try{const u=new URL(link.startsWith('@')?'https://t.me/'+link.slice(1):link.startsWith('t.me/')?'https://'+link:link),dest=new URL(a.url);if(u.origin!==dest.origin||(u.hostname==='t.me'?u.pathname.toLowerCase()!==dest.pathname.toLowerCase():u.pathname!==dest.pathname))add(base+'.text','link_destination','Ссылка в тексте должна вести туда же, куда URL объявления',undefined,undefined,'telegram_guidelines');}catch{add(base+'.text','link_format','Некорректная ссылка в тексте');}}
 }
 ...
 if(a.websiteName!=null&&(typeof a.websiteName!=='string'||!a.websiteName.trim()||a.websiteName.length>128||destinationKind(a.url)!=='website'))add(base+'.websiteName','website_name','websiteName: непустой заголовок до 128 символов только для сайта');
