...
const $=id=>document.getElementById(id);let data=null,receipts={},events=[],busy=false;
const extension=typeof chrome!=='undefined'&&!!chrome.storage;
const store=async()=>{const state={data,receipts,events};if(extension)await chrome.storage.local.set({state});else localStorage.setItem('tg-assistant-demo',JSON.stringify(state));};
...
$('example').onclick=async()=>{try{await load(await (await fetch('fixtures/example.json')).json());}catch(e){note(e.message);}};
$('inspect').onclick=async()=>{try{const result=await invoke('inspect');log('inspect',result);await store();note('Структура открытой формы записана в журнал.');}catch(e){note(e.message);}};
$('export').onclick=()=>{const u=URL.createObjectURL(new Blob([JSON.stringify({version:1,package:data,receipts,events},null,2)],{type:'application/json'}));const a=document.createElement('a');a.href=u;a.download='telegram-ads-receipts.json';a.click();setTimeout(()=>URL.revokeObjectURL(u),1000);};
$('clear').onclick=async()=>{if(!confirm('Удалить локальный пакет и журнал? Объявления в Telegram не изменятся.'))return;data=null;receipts={};events=[];await store();$('log').textContent='Нет действий';draw();note('Локальные данные удалены.');};
try{const saved=extension?(await chrome.storage.local.get('state')).state:JSON.parse(localStorage.getItem('tg-assistant-demo')||'null');if(saved){if(saved.data&&validatePackage(saved.data).length===0)data=saved.data;receipts=saved.receipts||{};events=saved.events||[];$('log').textContent=JSON.stringify(events,null,2);}draw();}catch(e){note('Не удалось восстановить локальные данные: '+e.message);}
