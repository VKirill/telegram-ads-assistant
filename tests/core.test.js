...
test('rejects unsafe destinations and long payloads',()=>{for(const url of ['javascript:alert(1)','https://user:password@example.test/','https://t.me/ExampleBot?start='+ 'a'.repeat(65)]){const p=sample();p.ads[0].url=url;assert.ok(validatePackage(p).length);}});
